...
impl SendChannels {
    pub fn send<T, MS: MessagesSerializer<T>>(
        ...
        if high_priority {
            println!("AURELIEN: send high priority the channel is a len of {}", self.high_priority.len());
            self.high_priority.send(data).map_err(|err| {
                PeerNetError::SendError.new("send sendchannels highprio", err, None)
            })?;
        } else {
            println!("AURELIEN: send high priority the channel is a len of {}", self.low_priority.len());

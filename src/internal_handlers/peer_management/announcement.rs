...
impl Announcement {
    pub fn new(listeners: HashMap<SocketAddr, TransportType>, keypair: &KeyPair) -> Result<Self, PeerNetError> {
        ...
        let timestamp = SystemTime::now().duration_since(UNIX_EPOCH).expect("Time went backward").as_millis();
        buf.extend(timestamp.to_be_bytes());
        Ok(Self {
            listeners,
            timestamp,
            signature: keypair.sign(&Hash::compute_from(&buf)).map_err(|err| PeerNetError::SignError(err.to_string()))?,
            serialized: buf,
        })
    }
    ...
    pub fn from_bytes(bytes: &[u8], peer_id: &PeerId) -> Result<Self, PeerNetError> {
        ...
        for _ in 0..nb_listeners {
            let ip: IpAddr = match bytes[i] {
                ...
                _ => return Err(PeerNetError::HandshakeError("Invalid IP version".to_string())),
            };
            ...
            let transport_type = match bytes[i] {
                ...
                _ => return Err(PeerNetError::HandshakeError("Invalid transport type".to_string())),
            };
            i += 1;

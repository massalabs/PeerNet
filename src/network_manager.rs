...
impl ActiveConnections {
    ...
    pub fn check_addr_accepted(&self, addr: &SocketAddr) -> bool {
        println!("check_addr_accepted ip to compare: {:?}", to_canonical(addr.ip()));
        for addr in &self.connection_queue {
            println!("check_addr_accepted ip existing: {:?}", to_canonical(addr.ip()));
        }
        for (id, connection) in &self.connections {
            println!("check_addr_accepted ip existing in connections: {:?}", to_canonical(connection.endpoint.get_target_addr().ip()));
